...
function ModifierUtilisateur($nom, $prenom, $date, $email, $pseudo, $mdp, $description) {

    $req = getConnexionBDD()->prepare("UPDATE user SET Nom=:nom, Prenom=:prenom, DateNaissance=:date, Email=:email, Pseudo=:pseudo, MotDePasse=SHA1(:mdp), Description=:description");
    $req->bindParam(':nom', $nom, PDO::PARAM_STR);
    $req->bindParam(':prenom', $prenom, PDO::PARAM_STR);
    $req->bindParam(':date', $date, PDO::PARAM_STR);
    $req->bindParam(':email', $email, PDO::PARAM_STR);
